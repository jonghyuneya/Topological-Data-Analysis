...
CACHE_FILE = CACHE_DIR / "gasteiger_dist_cache.pt"


# =====================================================================
# 1. RDKit 기반 3D feature 계산
# =====================================================================
def compute_mol_features(smiles: str, seed: int = 42):
    """
    한 분자에 대해:
...
def load_and_preprocess(args, device):
    ...
    # ---- subset 선택 (파이프라인 빠른 검증용) ----
    if args.subset is not None and args.subset < len(dataset):
        rng = np.random.default_rng(42)
        chosen = set(rng.choice(len(dataset), size=args.subset, replace=False).tolist())
        train_idx = [i for i in split_idx["train"].tolist() if i in chosen]
        valid_idx = [i for i in split_idx["valid"].tolist() if i in chosen]
        test_idx  = [i for i in split_idx["test"].tolist()  if i in chosen]
        print(f"      [subset 모드] {args.subset}개로 축소 "
              f"(train={len(train_idx)}, val={len(valid_idx)}, test={len(test_idx)})")
    else:
    ...
    if args.no_3d_features:
        ...
        print(f"[2/4] Partial charge + 3D 결합거리 계산 중... (대상 {len(used_idx)}개)")
        # SMILES 매핑 로드
        ...
        for k, i in enumerate(used_idx):
            ...
            feats = compute_mol_features(smiles)
            cache[i] = feats
            new_count += 1
            if new_count % 200 == 0:
